...
def get_qdrant_clients():
    ...
    # 1. Try remote
    try:
        remote_c = QdrantClient(host='localhost', port=6333, check_compatibility=False, timeout=3.0)
        remote_c.get_collections()
        clients.append(("Uzak Qdrant (localhost:6333)", remote_c))
    except Exception:
        pass

    # 2. Local embedded
    try:
        local_c = QdrantClient(path="data/qdrant_db")
        clients.append(("Yerel Qdrant (data/qdrant_db)", local_c))
    except Exception:
        pass
    return clients
...
def main():
    ...
    archive_path = "data/pedagogy/raw_teacher_cards_archive.jsonl"
    os.makedirs(os.path.dirname(archive_path), exist_ok=True)

    clients = get_qdrant_clients()
    if not clients:
        print("[!] Hiçbir Qdrant istemcisine ulaşılamadı!")
        return
    ...
    for client_name, client in clients:
        ...
        for col_name in ["kristal_bellek", "simulasyon_bellek"]:
            ...
            with open(archive_path, "a", encoding="utf-8") as archive_f:
                while True:
                    ...
                    for p in res:
                        ...
                        if cot_regex.search(txt) or "<think>" in txt or "<thought>" in txt:
                            # Archive raw teacher output safely for future training datasets
                            archive_entry = {
                                "client": client_name,
                                "collection": col_name,
                                "id": str(p.id),
                            ...
                            archive_f.write(json.dumps(archive_entry, ensure_ascii=False) + "\n")
                            archived_count += 1
            ...
            if archived_count > 0:
                print(f"  * Gelecek eğitimler için arşivlendi -> {archive_path} ({archived_count} kayıt)")
            if points_to_delete:
                print(f"  * Aktif RAG indeksinden kaldırılan kirli kayıt: {len(points_to_delete)} adet")
                client.delete(col_name, points_selector=points_to_delete)
            if points_to_update:
                print(f"  * Temizlenip RAG indeksinde güncellenen: {len(points_to_update)} adet.")
