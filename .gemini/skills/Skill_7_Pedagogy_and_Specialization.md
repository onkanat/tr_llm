...
*   **Hacim:** 3.682 örnek (`data/pedagogy/infancy_dataset.jsonl`).
*   **Örnek:** `Top: Pozitif: Top seker. Negatif: *Top içilir.`

### 2. Faz 2: Ebeveynlik (Parenting) - Derin Morfolojik ve Anlamsal Görevler
*   **Derinleştirilmiş 10 Morfolojik Görev:** Kök bulma, çoğul tespiti, hâl tespiti, zaman kipi, çatı dönüşümleri, olumsuzluk, soru eki, türetim vb. (40.001 örnek, `data/pedagogy/parenting_deep_dataset.jsonl`).
*   **TDK GTS Anlamsal Kök Sözlüğü:** 25.000 temel kökün sözlük tanımları ve semantik alanları (`data/pedagogy/lexical_semantics_dataset.jsonl`).

### 3. Faz 3: Sosyalleşme (Socialization) - Zanaat, Diyalog ve Otonom RAG
*   **Alan Uzmanlığı (Marangozluk & Zanaat):** 5.400 örnek ahşap işleme terminolojisi ve zanaat mantığı (`data/pedagogy/carpenter_specialization_dataset.jsonl`).
*   **Doğal Türkçe Sohbet & Diyalog:** 6.000 çok turlu doğal konuşma örneği (`scripts/generate_deep_chat_dataset.py`, `data/pedagogy/chat_conversations.jsonl`).
*   **Otonom Self-RAG Etkileşimi:** Modelin bilgi açığını algılayıp `<ARA>` ve `<BELGE>` etiketleriyle çalıştığı 6.599 interaktif senaryo (`scripts/generate_interactive_rag_dataset.py`, `data/pedagogy/rag_interactive_dataset.jsonl`).
...
| :--- | :--- | :--- |
| **Bebeklik (Infancy)** | `infancy_dataset.jsonl` | 3.682 |
| **Ebeveynlik Derin (Parenting Deep)** | `parenting_deep_dataset.jsonl` | 40.001 |
| **TDK GTS Anlamsal Sözlük** | `lexical_semantics_dataset.jsonl` | 25.000 |
| **Marangozluk Uzmanlığı** | `carpenter_specialization_dataset.jsonl` | 5.400 |
| **Doğal Türkçe Sohbet** | `chat_conversations.jsonl` | 6.000 |
| **Otonom Self-RAG** | `rag_interactive_dataset.jsonl` | 6.599 |
| **DPO Tercih Çiftleri (Aktif/Tüm)** | `dpo_tokenized.jsonl` / `dpo_all_tokenized.jsonl` | 321 / 6.198 |
...
Aktif 321 adet (toplam 6.198 aday arasından filtrelenen, `data/pedagogy/dpo_tokenized.jsonl`) tercih çifti (chosen vs. rejected) üzerinde modelin kaba veya halüsinatif ifadeler yerine akademik/zanaatkar ve doğru morfolojik yanıtları tercih etmesi sağlanır.
