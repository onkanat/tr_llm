...
* **`test_model.py` Boyut Uyumsuzluğu (T-0027 & T-0028) — ÇÖZÜLDÜ (16 Eyl 2026, T-0046):** Betik hâlâ sabit `data/vocab.json` (31.357) ile başlatıldığı için eski sınırlama `test_model.py`'de sürer; **ancak eğitim hattı artık kanonik taban sözlüğünü (`data/rebuild/vocab_base_32852.json`, **32.852**) yükler ve modeli ondan kurar**; checkpoint `resize_state_dict` ile hizalanır ve uyumsuzluk `[SOZLESME_UYARI]` satırıyla **görünür** biçimde basılır (sessiz kırpma yok).
* **Tabanın Noktalama/Rakam Eksikliği (D-A..D-E Kararları) — İLK İKİ ADIM KOŞULDU (16 Eyl 2026):** (i) F2 ile bayat `.bin`'ler yeni sözlükle yeniden derlendi (noktalama bloğu 32137-32145 artık **fiilen tensor'de**; `train_chat_balanced.bin` 180.336 noktalama, kesme 39.706); (ii) F3 ile tabana 2×1000 adımlık yetkinlik tamamlama koşumu yapıldı. **Hedef kapı (noktalama üretimi) HENÜZ SAĞLANMADI** ve koşumda `<PAD>` yozlaşması ölçüldü (kontrollü ikiz deney: maskeleme çözüyor) → sıradaki adım **maskeli yeniden koşum (`f3_clean`) + F4 ceket eğitimi**; `kristal_model_f3.pt` bu hâliyle F4 tabanı **olmamalıdır** ([[base_and_jacket_architecture]]).
